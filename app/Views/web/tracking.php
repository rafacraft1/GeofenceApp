...
<div class="grid grid-cols-1 lg:grid-cols-4 gap-6 h-[80vh]">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col h-full overflow-hidden">
        <div class="p-4 border-b bg-gray-50">
            ...
            <input type="text" id="search-siswa" placeholder="Cari nama/nis..." class="mt-3 w-full border-gray-200 rounded-lg p-2 text-sm outline-none focus:ring-2 focus:ring-blue-500 transition-all">
            ...
            <?php if (!empty($list_siswa)): ?>
                <?php foreach ($list_siswa as $s): ?>
                    <label class="flex items-center gap-3 p-3 hover:bg-blue-50 rounded-xl cursor-pointer transition-colors student-item">
                        <input type="checkbox" value="<?= esc((string) $s['id_siswa']) ?>" class="w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500 track-checkbox" <?= ((string) $target_id === (string) $s['id_siswa']) ? 'checked' : '' ?>>
                        <div>
                            <div class="text-sm font-bold text-gray-800 student-name"><?= esc((string) $s['nama_siswa']) ?></div>
                            <div class="text-xs text-gray-500 student-class"><?= esc((string) ($s['nama_kelas'] ?? '-')) ?> - <?= esc((string) $s['nis']) ?></div>
                        </div>
                    </label>
                <?php endforeach; ?>
            <?php else: ?>
                <div class="p-4 text-center text-sm text-gray-400">Belum ada data siswa.</div>
            <?php endif; ?>
        </div>
    </div>
...
<script>
    ...
    const firebaseUrl = "<?= esc((string) ($config['firebase_url'] ?? '')) ?>";
    let db = null;
    ...
    const sekolahLat = <?= esc((string) ($config['latitude_sekolah'] ?? 0)) ?>;
    const sekolahLon = <?= esc((string) ($config['longitude_sekolah'] ?? 0)) ?>;
    const radiusM = <?= esc((string) ($config['radius_meter'] ?? 0)) ?>;
    ...
    window.L.marker([sekolahLat, sekolahLon], {
        icon: schoolIcon
    }).addTo(map).bindPopup("<b>Lokasi Sekolah</b>");
