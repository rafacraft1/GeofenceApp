...
<body class="bg-gray-50 font-sans antialiased flex h-screen overflow-hidden">
    ...
    <aside id="sidebar" class="bg-slate-900 text-slate-300 w-64 flex-shrink-0 fixed md:relative inset-y-0 left-0 transform -translate-x-full md:translate-x-0 transition-all duration-300 ease-in-out z-50 flex flex-col shadow-2xl">
        ...
        <nav class="flex-1 px-3 py-6 space-y-1 overflow-y-auto">
            ...
            <p class="px-3 text-xs font-semibold text-slate-500 uppercase tracking-wider mt-4 mb-2">Data & Operasional</p>
            ...
            <a href="/admin/izin" class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors <?= $segment == 'izin' ? 'bg-blue-600 text-white shadow-md' : 'hover:bg-slate-800 hover:text-white' ?>">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                </svg>
                <span class="font-medium text-sm">Persetujuan Izin</span>
            </a>
            ...
            <a href="/admin/laporan" class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors <?= $segment == 'laporan' ? 'bg-blue-600 text-white shadow-md' : 'hover:bg-slate-800 hover:text-white' ?>">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <span class="font-medium text-sm">Rekap Laporan</span>
            </a>
            <a href="/admin/log-fraud" class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors <?= $segment == 'log-fraud' ? 'bg-red-600 text-white shadow-md' : 'hover:bg-slate-800 hover:text-white' ?>">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path>
                </svg>
                <span class="font-medium text-sm">Log Keamanan</span>
            </a>

            <?php if ($roleId === 1): ?>
                <p class="px-3 text-xs font-semibold text-slate-500 uppercase tracking-wider mt-6 mb-2">Administrator</p>
                ...
                <a href="/admin/user" class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors <?= $segment == 'user' ? 'bg-blue-600 text-white shadow-md' : 'hover:bg-slate-800 hover:text-white' ?>">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="/admin/libur" class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors <?= $segment == 'libur' ? 'bg-blue-600 text-white shadow-md' : 'hover:bg-slate-800 hover:text-white' ?>">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                        <circle cx="12" cy="14" r="2" fill="currentColor"></circle>
                    </svg>
                    <span class="font-medium text-sm">Hari Libur</span>
                </a>

                <a href="/admin/jadwal" class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors <?= $segment == 'jadwal' ? 'bg-blue-600 text-white shadow-md' : 'hover:bg-slate-800 hover:text-white' ?>">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
                    <span class="font-medium text-sm">Jadwal Harian</span>
                </a>
        ...
        <header class="h-16 bg-white shadow-sm border-b border-gray-200 flex items-center justify-between px-4 lg:px-6 z-30 relative">
            ...
            <div class="flex items-center gap-3">
                <div class="hidden sm:flex flex-col text-right">
                    ...
                    <span class="text-[10px] font-bold <?= $roleId === 1 ? 'text-indigo-600' : 'text-emerald-600' ?> uppercase tracking-wider"><?= esc((string) (session()->get('role') ?? 'User')) ?></span>
                </div>
